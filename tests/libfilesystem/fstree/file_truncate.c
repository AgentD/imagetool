...
int main(void)
{
	...
	/* truncate the sparse file and cut away tail + sparse block */
	ret = fstree_file_truncate(fs, f1, 3 * BLK_SIZE);
	TEST_EQUAL_I(ret, 0);

	TEST_STR_EQUAL(dummy_buffer, "X___AAAABBBBCCCCDDDDAAA_EEEE");
	...
	TEST_EQUAL_UI(f1->data.file.size, 12);
	...
	TEST_NULL(f1->data.file.sparse->next);
	...
	/* truncate right into the middle of a sparse region */
	ret = fstree_file_truncate(fs, f1, 4);
	TEST_EQUAL_I(ret, 0);

	TEST_STR_EQUAL(dummy_buffer, "X___AAAABBBBCCCCDDDDAAA_");
	TEST_EQUAL_UI(used, 6);
	TEST_EQUAL_UI(fs->data_offset, used);

	TEST_EQUAL_UI(f0->data.file.size, 19);
	TEST_EQUAL_UI(f0->data.file.start_index, 1);
	TEST_EQUAL_UI(f1->data.file.size, 4);
	TEST_EQUAL_UI(f1->data.file.start_index, 6);
	TEST_EQUAL_UI(f2->data.file.size, 1);
	TEST_EQUAL_UI(f2->data.file.start_index, 0);
	TEST_EQUAL_UI(f3->data.file.size, 2);
	...
	/* truncate that causes other files to move up */
	ret = fstree_file_truncate(fs, f2, 0);
	TEST_EQUAL_I(ret, 0);

	TEST_STR_EQUAL(dummy_buffer, "AAAABBBBCCCCDDDDAAA_");
	TEST_EQUAL_UI(used, 5);
	TEST_EQUAL_UI(fs->data_offset, used);
	...
	TEST_EQUAL_UI(f1->data.file.size, 4);
	TEST_EQUAL_UI(f1->data.file.start_index, 6);
	TEST_EQUAL_UI(f2->data.file.size, 0);
	TEST_EQUAL_UI(f2->data.file.start_index, 0);
	TEST_EQUAL_UI(f3->data.file.size, 2);
	TEST_EQUAL_UI(f3->data.file.start_index, 0);
	...
	/* truncate that reshapes the tail end */
	ret = fstree_file_truncate(fs, f0, 17);
	TEST_EQUAL_I(ret, 0);

	TEST_STR_EQUAL(dummy_buffer, "AAAABBBBCCCCDDDDA");
	TEST_EQUAL_UI(used, 5);
	TEST_EQUAL_UI(fs->data_offset, used);

	TEST_EQUAL_UI(f0->data.file.size, 17);
	TEST_EQUAL_UI(f0->data.file.start_index, 0);
	TEST_EQUAL_UI(f1->data.file.size, 4);
	TEST_EQUAL_UI(f1->data.file.start_index, 6);
	TEST_EQUAL_UI(f2->data.file.size, 0);
